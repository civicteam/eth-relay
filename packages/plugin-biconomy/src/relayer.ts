import {Relayer, RelayerBuilder, RelayResponse, RelayStatus} from "@civic/eth-relay";
import { Biconomy } from "@biconomy/mexa";
import {BigNumber, PopulatedTransaction, providers, Wallet} from "ethers";
...
export class BiconomyRelayer implements Relayer<RelayResponse, RelayStatus, BiconomyConfig> {
    constructor(
    ...
    static with(config: BiconomyConfig): RelayerBuilder<RelayResponse, RelayStatus, BiconomyConfig> {
            ...
            const web3provider: providers.ExternalProvider = {
                request: (request: { method: string, params?: Array<any> }) => {
                    if (request.method === "eth_signTypedData_v4") {
                        ...
                        return wallet._signTypedData(param.domain, param.types, param.message);
                    }
    ...
    async send(tx: PopulatedTransaction): Promise<RelayResponse> {
        ...
        return new Promise((resolve, reject) => {
            if (!this.biconomy.provider.send) return reject(new Error("Biconomy send function unavailable."));
            this.biconomy.provider.send({ method: "eth_sendTransaction", params: [txParams]}, (error, result) => {
                if (error) {
                    reject(error);
                } else {
                    resolve(result);
