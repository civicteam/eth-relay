...
export class ITXRelayer implements Relayer<RelayResponse, ITXRelayStatus, ITXConfig> {
    private provider: providers.InfuraProvider;
    private itxOptions: Options;
    constructor(
    ...
    static with(config: ITXConfig): RelayerBuilder<RelayResponse, ITXRelayStatus, ITXConfig> {
        return async (chainId: number, wallet: Wallet) => new ITXRelayer(chainId, wallet, config.apiKey, config.forwarderAddress, config.options)
    }
    ...
    async getBalance(): Promise<BigNumber> {
        const { balance } = await this.provider.send('relay_getBalance', [this.wallet.address]);

        return BigNumber.from(balance);
    }

    async lookup(task: string): Promise<ITXRelayStatus> {
        const statusResponse = await this.provider.send('relay_getTransactionStatus', [task]);
     ...
     * @private
     */
    private signRequest(tx: RelayRequest): Promise<string> {
    ...
    private async signMetaTx(tx: PopulatedTransaction): Promise<PopulatedTransaction> {
        if (!tx.data || !tx.to) throw new Error("ITX requires a data field and to address in the transaction.");
        const forwarderContract = new Contract(this.forwarderAddress, forwarderAbi).connect(this.wallet) as Forwarder;
    ...
    async send(tx: PopulatedTransaction): Promise<RelayResponse> {
        ...
        if (!metaTx.data || !metaTx.to) throw new Error("ITX requires a data field and to address in the meta transaction.");
        ...
        const {relayTransactionHash} = await this.provider.send('relay_sendTransaction', [request, signature]);
